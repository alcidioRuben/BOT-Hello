...
// 👇 ATUALIZAÇÃO COMPLETA DAQUI 👇
exports.activateGroup = (groupId, expiresAt = null) => {
  const filename = INACTIVE_GROUPS_FILE;

  let inactiveGroups = readJSON(filename, {});

  // Remove da lista de inativos
  delete inactiveGroups[groupId];

  // Gerencia expiração
  if (!inactiveGroups._expirations) {
    inactiveGroups._expirations = {};
  }

  if (expiresAt) {
    inactiveGroups._expirations[groupId] = expiresAt;
  } else {
    delete inactiveGroups._expirations[groupId];
  }
...
exports.deactivateGroup = (groupId) => {
  ...
  // Adiciona como inativo
  inactiveGroups[groupId] = null;

  // Remove expiração se existir
  if (inactiveGroups._expirations) {
    delete inactiveGroups._expirations[groupId];
  }
...
exports.isActiveGroup = (groupId) => {
  ...
  // Está na lista? Está OFF
  if (inactiveGroups[groupId] !== undefined) {
    return false;
  }

  // Verifica expiração
  if (inactiveGroups._expirations && inactiveGroups._expirations[groupId]) {
    const expiresAt = new Date(inactiveGroups._expirations[groupId]);
    const now = new Date();

    if (now > expiresAt) {
      // Expirou: marca como inativo
      inactiveGroups[groupId] = null;
      delete inactiveGroups._expirations[groupId];
      writeJSON(filename, inactiveGroups, {});
      return false;
...
exports.getGroupExpiration = (groupId) => {
  const filename = INACTIVE_GROUPS_FILE;
  ...
  return inactiveGroups._expirations[groupId] || null;
};
const ANTI_ESTRANGEIROS_FILE = "anti-estrangeiros";

exports.activateAntiEstrangeiros = (groupId) => {
  const filename = ANTI_ESTRANGEIROS_FILE;
...
exports.deactivateAntiEstrangeiros = (groupId) => {
  const filename = ANTI_ESTRANGEIROS_FILE;
...
exports.isActiveAntiEstrangeiros = (groupId) => {
  const filename = ANTI_ESTRANGEIROS_FILE;
